...
test("homepage and guest account surface clearer next steps", () => {
  ...
  assert.match(homepage, /Discover Roatan clearly/);
  assert.match(account, /Private guest lounge/);
  assert.match(account, /Start planning/);
});
...
test("big upgrade surfaces brand media, guest reviews, and vendor focus", () => {
  ...
  assert.match(
    account + readProjectFile("lib/guest-command-center.ts"),
    /Leave review/,
  );
  assert.match(vendorDashboard, /Growth checklist/);
...
test("next upgrade surfaces planner, command, and readiness polish", () => {
  ...
  assert.match(homepage, /Pick the pace\. Roa and the map will narrow the island\./);
  ...
  assert.match(guestDashboard, /Trip timeline/);
});
...
test("booking conversion pro surfaces trust, readiness, and recovery polish", () => {
  ...
  assert.match(bookPage, /Private booking concierge/);
  ...
  assert.match(successPage, /Trip request concierge/);
...
test("vendor payout statements include receipts, exports, and paid email flow", () => {
  ...
  assert.match(vendorDashboard, /payout statements/i);
...
test("booking change requests are available to guests, vendors, and admins", () => {
  ...
  assert.match(guestAccount, /change requests/);
  assert.match(
    readProjectFile("app/book/status/[id]/BookingChangeRequestForm.tsx"),
    /Request a change/,
  );
