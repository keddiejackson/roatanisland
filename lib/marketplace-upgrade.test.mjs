...
test("creates conversion tags from traveler intent and listing readiness", () => {
  assert.deepEqual(
    getListingConversionTags(
      {
        ...listing,
        ...
        description: "Airport pickup and West Bay dropoff.",
      },
      { date: "2026-06-16", guests: "4" },
    ),
    ["Open on your date", "Fits 4 guests", "Airport pickup", "Times listed"],
  );
});
...
test("builds an admin command digest with the strongest next action", () => {
  const digest = getAdminCommandDigest({
    listings: [
      listing,
      { ...listing, id: "listing-2", latitude: null, longitude: null },
    ],
    bookings: [{ id: "b1", status: "new", tour_date: "2026-06-10", guests: 2 }],
    vendors: [{ id: "v1", is_active: true }],
    reviews: [{ id: "r1", is_approved: false }],
    conciergeLeadCount: 2,
  });

  assert.equal(digest.headline, "5 items need attention");
  assert.equal(digest.topAction.label, "Concierge leads");
  assert.equal(digest.topAction.href, "/admin/concierge");
  assert.deepEqual(
    digest.items.map((item) => item.label),
    [
      "Concierge leads",
      "Missing map pins",
      "Pending reviews",
