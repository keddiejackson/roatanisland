...
test("scores listings with preferred time language", () => {
  const suggestions = getRoaSuggestedListings({
    listings: [
      {
        id: "sunset-reef",
        title: "Premium Reef Boat",
        category: "Private Charters",
        location: "West End",
        description: "Private reef charter.",
        price: 275,
        rating: 4.8,
        reviews_count: 3,
        tour_times: ["4:30 PM", "Sunset"],
        max_guests: 6,
      },
      {
        id: "morning-reef",
        title: "Morning Reef Boat",
        category: "Private Charters",
        location: "West End",
        description: "Private reef charter.",
        price: 275,
        rating: 5,
        reviews_count: 6,
        tour_times: ["8:30 AM", "Morning"],
        max_guests: 6,
      },
    ],
    latestMessage: "I need a private sunset option around 4:30 PM.",
  });

  assert.equal(suggestions[0].id, "sunset-reef");
  assert.ok(suggestions[0].reasons.some((reason) => /time|sunset/i.test(reason)));
});
...
test("builds Roa Trip Desk timeline and snapshot", () => {
  const suggestedListings = getRoaSuggestedListings({
    listings,
    latestMessage: "Plan a private sunset charter for 2 guests.",
    traveler: {
  ...
  const timeline = getRoaTripDeskTimeline("contacted");
  const snapshot = getRoaTripDeskSnapshot({
    plan,
    suggestedListings,
    traveler: {
      tripDate: "2026-07-10",
  ...
  assert.deepEqual(
    timeline.map((step) => `${step.key}:${step.state}`),
    [
      "draft:complete",
      "sent:complete",
      "reviewing:complete",
      "vendor_contacted:active",
      "quote_ready:upcoming",
      "booked:upcoming",
    ],
  );
  assert.equal(snapshot.statusLabel, "Vendor contacted");
  assert.equal(snapshot.missingDetails.length, 0);
  assert.match(snapshot.estimatedValueLabel, /\$/);
  assert.equal(snapshot.itineraryBlocks[0].title, "Private Sunset Boat");
  assert.match(snapshot.pickupSummary, /West End/);
});
