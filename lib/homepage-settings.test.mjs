...
test("defaults homepage hero copy to the luxury homepage wording", () => {
  ...
  assert.equal(
    ...
    "Plan with trusted local operators, exact island context, and Roa concierge help in one calm place.",
  );
...
test("uses a separate draft settings key for homepage previews", () => {
  assert.equal(publishedSiteSettingsKey, "site");
  assert.equal(draftSiteSettingsKey, "site_draft");
  assert.equal(getHomepageSettingsKey(false), "site");
  assert.equal(getHomepageSettingsKey(true), "site_draft");
});

test("builds homepage quality checklist warnings before publishing", () => {
  const controls = {
    ...normalizeHomepageControls({
      showFeaturedListings: true,
      homepageFeaturedListingIds: [],
      trustPoints: [{ title: "One", text: "Only one trust point" }],
    }),
    heroImageUrl: "",
    primaryCtaHref: "",
    secondaryCtaHref: "",
  ...
  const checklist = getHomepageQualityChecklist(controls, {
    availableListingCount: 0,
    selectedListingCount: 0,
  });

  assert.ok(checklist.some((item) => item.id === "hero-image"));
  assert.ok(checklist.some((item) => item.id === "button-links"));
  assert.ok(checklist.some((item) => item.id === "featured-listings"));
  assert.ok(checklist.some((item) => item.id === "trust-points"));
});
