...
test("regular website desktop navigation uses the guest platform menu", () => {
  ...
  assert.match(nav, /href: "\/community", label: "Circle"/);
...
test("category and vendor list headers use readable desktop nav on light pages", () => {
  const categoryPage = read("app/category-page.tsx");
  const vendorsPage = read("app/vendors/page.tsx");

  assert.match(categoryPage, /<GuestDesktopNav \/>/);
  assert.match(vendorsPage, /<GuestDesktopNav \/>/);
  assert.doesNotMatch(categoryPage, /<GuestDesktopNav variant="light" \/>/);
  assert.doesNotMatch(vendorsPage, /<GuestDesktopNav variant="light" \/>/);
});

test("category pages use natural marketplace labels", () => {
  const categoryPage = read("app/category-page.tsx");

  assert.match(categoryPage, /browseTitle: "Hotels & stays"/);
  assert.match(categoryPage, /emptyTitle: "No active hotels or stays yet\."/);
  assert.match(categoryPage, /browseTitle: "Transportation options"/);
  assert.match(categoryPage, /emptyTitle: "No active transportation options yet\."/);
  assert.doesNotMatch(categoryPage, /\{category\} experiences/);
});
...
test("homepage, listing pages, and floating helpers use regular-site labels", () => {
  ...
  assert.match(homeHeader, />\s*Circle\s*</);
