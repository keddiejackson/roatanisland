...
test("homepage owns its header sign out without the floating chip", () => {
  const source = read("app/page.tsx") + read("app/HomeHeroHeader.tsx");
...
test("listing detail page exposes conversion tools", () => {
  ...
  assert.match(source, /Save listing/);
  assert.match(source, /Compare quickly/);
  assert.match(source, /\/book\?listing=/);
});
...
test("admin listing actions stack safely on mobile", () => {
  const source = read("app/admin/listings/page.tsx");

  assert.match(source, /overflow-x-hidden/);
  assert.match(source, /pb-36/);
  assert.match(source, /sm:grid-cols-2 lg:grid-cols-\[minmax\(0,1fr\)_minmax\(0,1fr\)_auto_auto\]/);
  assert.match(source, /Save changes/);
  assert.ok(
    source.lastIndexOf("Description") < source.lastIndexOf("Permanent delete"),
  );
});
